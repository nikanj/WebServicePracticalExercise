...
public class Stax {
	...
	/**
	 * @param xsdFileResource
	 *            the xsdFileResource to set
	 */
	...
	public void createWriter() {
		try {
			...
			writer = xmlOutputFactory.createXMLStreamWriter(new FileWriter(
					tempfileResource));
			eventWriter = xmlOutputFactory.createXMLEventWriter(new FileWriter(
					tempfileResource));
	...
	public void createReader() {
		try {
			...
			reader = xmlInputFactory.createXMLStreamReader(new FileReader(
	...
	public Object createReader(File fileResource, boolean isEventReader) {
		...
		try {
			if (!isEventReader) {
				return (xmlInputFactory.createXMLStreamReader(new FileReader(
						fileResource)));
			} else {
				return (xmlInputFactory.createXMLEventReader(new FileReader(
						fileResource)));
			}
		} catch (XMLStreamException | IOException e) {
			e.printStackTrace();
		...
		try {
			...
			if (!isEmptyDocument) {
				...
				while (eventReader.hasNext()) {
					...
					if (event.getEventType() == XMLStreamConstants.START_ELEMENT) {
						...
						if (startElement
								...
								.getLocalPart()
								.equalsIgnoreCase(
										XMLConstant.GUEST_BOOK_MAIN_SUFFIX)) {
							nodeinsert++;
						}
				...
				StartDocument startDocument = xmlEventFactory
						.createStartDocument("UTF-8", "1.0");
				eventWriter.add(startDocument);
				// eventWriter.add(end);

				Namespace ns1 = xmlEventFactory.createNamespace("k",
						"http://dss.in.tum.de/keyword");
				Namespace ns2 = xmlEventFactory.createNamespace("entry",
						"http://dss.in.tum.de/guestbooktype");
				Namespace ns3 = xmlEventFactory.createNamespace("gb",
						"http://dss.in.tum.de/guestbook");
				Namespace ns4 = xmlEventFactory.createNamespace("xsi",
						"http://www.w3.org/2001/XMLSchema-instance");
				Namespace ns5 = xmlEventFactory.createNamespace(
						"schemaLocation",
						"http://dss.in.tum.de/guestbook xsd/guestbook.xsd");

				List<Namespace> nsList = Arrays.asList(ns1, ns2, ns3, ns4, ns5);

				XMLEvent startElement = xmlEventFactory.createStartElement(
						new QName(XMLConstant.GUEST_BOOK_ENTRY_FIRST_ELEMENT),
						null, nsList.iterator());
				eventWriter.add(startElement);
				// eventWriter.add(end);

				createNode(create);

				XMLEvent endElement = xmlEventFactory.createEndElement(
						new QName(XMLConstant.GUEST_BOOK_ENTRY_FIRST_ELEMENT),
						nsList.iterator());
	...
	public void validateXMLFile(XMLStreamReader reader) throws SAXException,
	...
	private void createNode(GuestBook create) {
		...
		try {
			...
			XMLEvent event = xmlEventFactory.createStartElement(
					XMLConstant.GUEST_BOOK_MAIN_PREFIX, "",
					XMLConstant.GUEST_BOOK_ENTRY_SUFFIX);

			eventWriter.add(event);

			createGuestBookElementTag(XMLConstant.GUEST_BOOK_ELEMENT_PREFIX,
					XMLConstant.BOOK_ID_SUFFIX, "" + create.getGuestBookId());
			createGuestBookElementTag(XMLConstant.GUEST_BOOK_ELEMENT_PREFIX,
					XMLConstant.AUTHOR_TITLE_SUFFIX, create.getTitle());
			createGuestBookElementTag(XMLConstant.GUEST_BOOK_ELEMENT_PREFIX,
					XMLConstant.AUTHOR_NAME_SUFFIX, create.getName());
			createGuestBookElementTag(XMLConstant.GUEST_BOOK_ELEMENT_PREFIX,
					XMLConstant.BOOK_DATA_SUFFIX, create.getText());

			if (create.getEmail() != null
					&& !create.getEmail().trim().isEmpty()) {
				createGuestBookElementTag(
						XMLConstant.GUEST_BOOK_ELEMENT_PREFIX,
						XMLConstant.GUEST_EMAIL_ID_SUFFIX, create.getEmail());
			}
			createGuestBookElementTag(XMLConstant.GUEST_BOOK_ELEMENT_PREFIX,
					XMLConstant.DATE_OF_CREATION_SUFFIX,
					GuestBookAdditional.formatDateStr(create.getCreateDate()));
			createGuestBookElementTag(XMLConstant.GUEST_BOOK_ELEMENT_PREFIX,
					XMLConstant.LAST_MODIFICATION_DATE_SUFFIX,
					GuestBookAdditional.formatDateStr(create
							.getLastModifiedDate()));
	...
	private void createGuestBookElementTag(String prefix, String suffix,
	...
	private void createGuestBookElementKeywordsTag(List<String> keywordList)
		 ...
		 * xmlEventFactory.createDTD("\t ");
		 */
		XMLEvent keywords = xmlEventFactory.createStartElement(
				XMLConstant.GUEST_BOOK_ELEMENT_PREFIX, "",
				XMLConstant.GUEST_BOOK_KEYWORDS_SUFFIX);
		// eventWriter.add(tab);
		// eventWriter.add(tab);
		...
		for (String keyword : keywordList) {
			...
			if (keyword != null && !keyword.trim().isEmpty()) {
				// eventWriter.add(tab);
				createGuestBookElementTag(
						XMLConstant.GUEST_BOOK_KEYWORD_PREFIX,
						XMLConstant.GUEST_BOOK_KEYWORD_SUFFIX, keyword);
			}
		}

		keywords = xmlEventFactory.createEndElement(
				XMLConstant.GUEST_BOOK_ELEMENT_PREFIX, "",
				XMLConstant.GUEST_BOOK_KEYWORDS_SUFFIX);
		// eventWriter.add(tab);
		// eventWriter.add(tab);
				...
				if (event.getEventType() == XMLStreamConstants.START_ELEMENT) {
					...
					if (startElement
						...
						if (idevent.getEventType() == XMLStreamConstants.START_ELEMENT) {
							StartElement idstartElement = idevent
									.asStartElement();
							...
							if (idstartElement
								...
								if (idValueEvent.getEventType() == XMLStreamConstants.CHARACTERS) {
									String idStr = idValueEvent.asCharacters()
											.getData();
									// System.out.println("Ids:"+idStr);
									int id = Integer.parseInt(idStr);

									if (idFound == 0
											&& id == delete.getGuestBookId()) {
										idFound++;
									}
								}
							}
