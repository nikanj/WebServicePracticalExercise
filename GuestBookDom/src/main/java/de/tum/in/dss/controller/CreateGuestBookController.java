...
public class CreateGuestBookController extends SimpleFormController {
	...
	protected ModelAndView onSubmit(HttpServletRequest request,
		...
		try{
			...
			guestBookRequest.setKeywordList(GuestBookAdditional.separateKeywordsToList(guestBookRequest.getKeywords(), CommonConstants.COMMA));
			guestBookRequest.setKeywordList(GuestBookAdditional.removeDuplicates(guestBookRequest.getKeywordList()));
	...
	protected Object formBackingObject(HttpServletRequest request)
		...
		if(param1!=null){
			try{
				...
				if(temp!=null){
					temp.setKeywords(GuestBookAdditional.listToKeyWords(temp.getKeywordList(), CommonConstants.COMMA));
				}
			}
			catch(Exception e){
				e.printStackTrace();
