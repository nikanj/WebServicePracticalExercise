...
public class GuestBookDaoImpl implements GuestBookDao {
	...
	@Override
	public int createGuestBookEntry(GuestBook create) throws SQLException {
		int created = 0;
		...
		created  = this.jdbcTemplate.update(sqlQueryCreate1, new Object[] {create.getName(), create.getTitle(),create.getText(),create.getEmail(),Calendar.getInstance().getTime(),Calendar.getInstance().getTime()});
	...
	public int[] insertKeywords(final List<String> keywords) {
		...
		int[] rowsUpdated = this.jdbcTemplate.batchUpdate(sqlQueryCreate2, new BatchPreparedStatementSetter() {
	...
	public int updateKeyword(Keyword keywordUpdate,int keywordId) {
		...
		updated  = this.jdbcTemplate.update(sqlQueryUpdate, new Object[] {keywordUpdate.getKeywordValue(),keywordId});
	...
	public void mappingGuestBookKeywords(GuestBook guestBookToMap) {
		...
		final List<Keyword> keywordObjList = retrieveKeywords(guestBookToMap.getKeywordList());
		// remove old mappings
		...
		if(keywordObjList!=null && keywordObjList.size()>0){
			//insert new mappings
			this.jdbcTemplate.batchUpdate(sqlQueryCreate2, new BatchPreparedStatementSetter() {
	...
	public int removeOrphanKeywords() {
		...
		if (orphanedkeywordIdList != null && orphanedkeywordIdList.size() > 0) {
			Map<String, List<Integer>> params = Collections.singletonMap("keywords", orphanedkeywordIdList);
			deleted = this.namedParameterJdbcTemplate.update(sqlQuerydelete, params);
		}
	...
	public int removeKeywordMappingByGuestBookId(int guestBookId) {
		...
		int deleted = this.jdbcTemplate.update(sqlQuerydelete, new Object[]{guestBookId});
	...
	public List<Keyword> retrieveKeywords(final List<String> keywords) {
		...
		if (keywords != null && keywords.size() > 0) {
			Map<String, List<String>> params = Collections.singletonMap("keywords", keywords);

			// SqlParameterSource namedParameters = new
			// BeanPropertySqlParameterSource(keywords);
			Object obj = this.namedParameterJdbcTemplate.query(sql, params,	new KeywordMapper());
	...
	public List<GuestBook> retrieveGuestBookWithKeywords(GuestBook guestBook) {
	    ...
	    if(obj!=null){
	    	...
	    	while(guestBookIt!=null && guestBookIt.hasNext()){
	    		...
	    		List<String> keywordsList = retrieveGuestBookKeywordsByGuestBookId(book.getGuestBookId());
	...
	public GuestBook retrieveGuestBookByGuestBookId(int guestBookId) {
		...
		GuestBook book = null;
		 Object obj =  this.jdbcTemplate.query(sql, new Object[]{guestBookId}, new GuestBookMapper());
		 if(obj!=null){
	...
	public List<GuestBook> retrieveGuestBookListByKeyword(String keyword) {
		...
		List<GuestBook> guestBookList = null;
	    Object obj = this.jdbcTemplate.query(sql, new Object[]{keyword}, new GuestBookMapper());
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<GuestBook> retrieveGuestBookListByKeywords(final List<String> keywordList) {
		String sql = "select * from GUEST_BOOK where BOOK_ID in (select book_id from GUEST_BOOK_KEYWORD_MAPPING where keyword_id in ( "+
		...
		if(keywordList!=null && keywordList.size()>0){
			Map<String, List<String>> params = Collections.singletonMap("keywords", keywordList);

			Object obj = this.namedParameterJdbcTemplate.query(sql, params,	new GuestBookMapper());
			List<GuestBook> list1 = null;
			if(obj!=null){
			    	list1 = (List<GuestBook>) obj;
			    	guestBookList.addAll(list1);
			}
			List<GuestBook> list2 = retrieveGuestBookListByKeywordsInText(keywordList);
			guestBookList.addAll(list2);
	...
	@SuppressWarnings("unchecked")
	public List<GuestBook> retrieveGuestBookListByKeywordsInText(final List<String> keywordList) {
		String sql ="select * from GUEST_BOOK where BOOK_DATA like ?";
		
		List<GuestBook> guestBookList = new ArrayList<GuestBook>();
		
		for(String keywordlike : keywordList){
			Object obj = this.jdbcTemplate.query(sql,new Object[] { CommonConstants.LIKE_OP + keywordlike + CommonConstants.LIKE_OP }, new GuestBookMapper());
			if(obj!=null){
		    	guestBookList = (List<GuestBook>) obj;
			}
		}
	...
	public List<String> retrieveGuestBookKeywordsByGuestBookId(int guestBookId) {
		...
		//Map<String, List<Integer>> params = Collections.singletonMap("guestBookId", guestBookId);
		
		List<String> guestBookKeywordsList = this.jdbcTemplate.query(sql,new Object[] { guestBookId }, new RowMapper() {
	...
	public List<String> retrieveGuestBookKeywords(List<String> keywordLikeList) {
		...
		for(String keywordlike : keywordLikeList){
			List<String> guestBookKeywordsList = this.jdbcTemplate.query(sql,new Object[] { CommonConstants.LIKE_OP + keywordlike + CommonConstants.LIKE_OP }, new RowMapper() {
						public Object mapRow(ResultSet resultSet, int i)
								throws SQLException {
							return resultSet.getString(1);
						}
	...
	public int deleteGuestBook(GuestBook delete) {
		...
		deleted = this.jdbcTemplate.update("DELETE from GUEST_BOOK where BOOK_ID = ?",new Object[] { delete.getGuestBookId()});
	...
	private static final class GuestBookMapper implements RowMapper {
		  ...
		  public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
	            ...
	            book.setCreateDate(new java.sql.Date(rs.getTimestamp("DATE_OF_CREATION").getTime()));
	            book.setLastModifiedDate(new java.sql.Date(rs.getTimestamp("LAST_MODIFICATION_DATE").getTime()));
	...
	@Override
	public int updateGuestBookEntry(GuestBook updatedGuestBook,int guestBookId) throws SQLException {
		int updated = 0;
		...
		updated  = this.jdbcTemplate.update(sqlQueryUpdate, new Object[] {updatedGuestBook.getName(), updatedGuestBook.getTitle(),updatedGuestBook.getText(),updatedGuestBook.getEmail(),Calendar.getInstance().getTime(),guestBookId});
		...
		// update mappings too
		if(updatedGuestBook.getKeywordList()!=null && updatedGuestBook.getKeywordList().size()>0){
			try {
	...
	public List<Keyword> retrieveGuestBookKeywords(String keywordLike) {
		...
		List<Keyword> guestBookKeywordsList = this.jdbcTemplate.query(sql,new Object[] { CommonConstants.LIKE_OP + keywordLike + CommonConstants.LIKE_OP }, new KeywordMapper());
		
		guestBookKeywordsList = GuestBookHelper.removeDuplicateKeywordObjects(guestBookKeywordsList);
	    return guestBookKeywordsList;
	}

	@Override
	public int updateGuestBookEntry(GuestBook update) throws SQLException {
