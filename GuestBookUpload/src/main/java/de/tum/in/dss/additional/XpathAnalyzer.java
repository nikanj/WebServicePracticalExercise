...
public class XpathAnalyzer {
	...
	private DomAdditional domadditional;
	...
	public DomAdditional getdomadditional() {
		return domadditional;
	}

	
	public void setdomadditional(DomAdditional domadditional) {
		this.domadditional = domadditional;
	}
	...
	public List<GuestBook> analyzeFile(MultipartFile file) throws IOException, XPathExpressionException{
		...
		Document doc = domadditional.getParseDocument(documentBuilder, file.getInputStream());
		setDocument(doc);
	...
	public List<GuestBook> parseGuestBookObject() throws XPathExpressionException {
		...
		if(result!=null){
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				String idStr = getTagValue("//guestbook/entry["+(i+1)+"]/BOOK_ID/text()");
				int id = Integer.parseInt(idStr);
				book.setGuestBookId(id);
				
				book.setTitle(getTagValue("//guestbook/entry["+(i+1)+"]/AUTHOR_TITLE/text()"));
				book.setName(getTagValue("//guestbook/entry["+(i+1)+"]/AUTHOR_NAME/text()"));
				book.setText(getTagValue("//guestbook/entry["+(i+1)+"]/BOOK_DATA/text()"));
				book.setEmail(getTagValue("//guestbook/entry["+(i+1)+"]/GUEST_EMAIL_ID/text()"));
				
				Date createdate = DatatypeConverter.parseDate(getTagValue("//guestbook/entry["+(i+1)+"]/DATE_OF_CREATION/text()")).getTime();
