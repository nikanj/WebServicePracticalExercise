...
public class GuestBookXMLDaoImpl implements GuestBookDao {
	...
	private DomAdditional domHelper;
	 ...
	 */
	public DomAdditional getDomHelper() {
		return domHelper;
	}

	/**
	 * @param domHelper
	 *            the domHelper to set
	 */
	public void setDomHelper(DomAdditional domHelper) {
	...
	public List<GuestBook> retrieveGuestBook(GuestBook guestBook) {
		...
		for (int entryIndex = 0; entryIndex < nList.getLength(); entryIndex++) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				guestBookList.add(domHelper.getGuestBookWithKeywords(
						getDocument(), entryIndex, eElement));
			}
		}
	...
	public int deleteGuestBook(GuestBook delete) {
		// Node nodeToDelete =
		// getGuestBookNodeElementByGuestBookId(delete.getGuestBookId());
		int deleted = 1;
	...
	public List<String> retrieveGuestBookKeywords(List<String> keywordLikeList) {
		...
		for (String keywordlike : keywordLikeList) {
			List<String> matchedList = GuestBookAdditional.getMatchingStrings(
					tempKeywordList, keywordlike);
	...
	public int updateGuestBookEntry(GuestBook updatedGuestBook, int guestBookId) {
		...
		if (bookFound != null) {
			...
			updatedGuestBook.setLastModifiedDate(bookFound
					.getLastModifiedDate());
	...
	public GuestBook retrieveGuestBookByGuestBookId(int guestBookId) {
		...
		for (int entryIndex = 0; entryIndex < nList.getLength(); entryIndex++) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				Element eElement = (Element) nNode;
				GuestBook tempbook = domHelper.getGuestBookWithKeywords(
						getDocument(), entryIndex, eElement);
	...
	public GuestBook retrieveGuestBookByGuestBookName(String guestBookName) {
		...
		for (int entryIndex = 0; entryIndex < nList.getLength(); entryIndex++) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (tempbook.getName() != null && guestBookName != null
						&& guestBookName.trim().equals(tempbook.getName())) {
					return tempbook;
				}
			}
		}
	...
	public List<GuestBook> retrieveGuestBookListByKeywords(
		...
		for (int entryIndex = 0; entryIndex < nList.getLength(); entryIndex++) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				Element eElement = (Element) nNode;
				GuestBook book = domHelper.getGuestBookWithKeywords(
						getDocument(), entryIndex, eElement);

				for (String keywordlike : keywordList) {
					List<String> matchedList = GuestBookAdditional
							.getMatchingStrings(book.getKeywordList(),
									keywordlike);
		...
		guestBookList = GuestBookAdditional
				.removeDuplicateGuestBookObjects(guestBookList);
		return guestBookList;
	}
		...
		for (int entryIndex = 0; entryIndex < nList.getLength(); entryIndex++) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				for (String keywordlike : keywordList) {
					...
					// GuestBookHelper.getExactMatchingStrings(book.getKeywordList(),keywordlike);

					if (keywordlike != null && book.getText() != null
							&& book.getText().contains(keywordlike)) {
						guestBookList.add(book);
					}
				}
			}
	...
	private Document getParseDocument() {
		...
		Document doc = null;
		try {
			doc = domHelper.getParseDocument(documentBuilder,
	...
	private List<String> getAllDistinctKeywordList() {
		...
		NodeList keyNodeList = getDocument().getElementsByTagName(
				XMLConstant.GUEST_BOOK_KEYWORDS);
		List<String> keywordList = null;
		int entryIndex = 0;

		while (keyNodeList.getLength() > 0
				&& entryIndex < keyNodeList.getLength()) {
			...
			for (int temp1 = 0; temp1 < nNodeList.getLength(); temp1++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					// +":::"+nNode.getChildNodes().item(0).getNodeValue());
					if (!keywordList.contains(nNode.getChildNodes().item(0)
							.getNodeValue())) {
						keywordList.add(nNode.getChildNodes().item(0)
								.getNodeValue());
					}
				}
			}
	...
	public int getMaxGuestBookId() {
		...
		NodeList idNodeList = getDocument().getElementsByTagName(
				XMLConstant.BOOK_ID);
		int entryIndex = 0;
		int maxGuestBookId = 0;

		while (idNodeList.getLength() > 0
				&& entryIndex < idNodeList.getLength()) {
